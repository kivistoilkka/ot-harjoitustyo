...
class TalentRespository:
    ...
    def find_for_archetype(self, archetype: str) -> dict:
        talents = list(filter(lambda talent: talent.starting_option_for == archetype, self.find_all()))
        talents_dict = {}
    ...
    def _read(self) -> list:
        ...
        for talent in talents_list:
            talent_object = Talent(talent["name"], talent["description"], talent["startingOptionFor"])
...
if __name__ == "__main__":
    repository = TalentRespository(TALENTS_FILE_PATH)
    talents = repository.find_all()
    print(talents)
    print(talents["Bookworm"])